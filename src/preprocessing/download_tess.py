...
DEFAULT_DATASET_NAME = "AbstractTTS/TESS"
...
def _all_dataset_items(dataset_name: str, split: str | None):
    if dataset_name != DEFAULT_DATASET_NAME:
        raise ValueError(
            f"Only the TESS mirror '{DEFAULT_DATASET_NAME}' is supported, got {dataset_name!r}"
        )
...
def download_tess(
    ...
    """Download the AbstractTTS/TESS speech emotion mirror from Hugging Face.
    ...
    for split_name, split_dataset in _all_dataset_items(dataset_name, split):
        ...
        for index, example in enumerate(
            tqdm(split_dataset, desc=f"Writing TESS WAV files ({split_name})")
        ):
            audio = example["audio"]
            file_name = str(example.get("file") or f"{split_name}_{index:05d}.wav")
